...
public class SimpleConfiguration implements Configuration, Cloneable {
    ...
    public boolean setRunOn(VirtualMachine vm, Node node) {
        return nodeState.get(node.hashCode()) == ONLINES && replace(vm, node, RUNNINGS);
    }

    @Override
    public boolean setSleepOn(VirtualMachine vm, Node node) {
        return nodeState.get(node.hashCode()) == ONLINES && replace(vm, node, SLEEPINGS);
    }
    ...
    public Configuration clone() {
        ...
        /*
        c.allVMs = allVMs.clone();
        c.allNodes = allNodes.clone();
        c.nodeState = (TIntIntHashMap) nodeState.clone();
        c.vmState = (TIntIntHashMap) vmState.clone();

        for (int i = 0; i < hosted.length; i++) {
            if (hosted[i] != null) {
                c.hosted[i] = hosted[i].clone();
            }
        }
        vmPlace.forEachEntry(new TIntObjectProcedure<Node>() {
            @Override
            public boolean execute(int i, Node node) {
                c.vmPlace.put(i, node);
                return true;
            }
        });
        for (int i = 0; i < vmsByState.length; i++) {
            c.vmsByState[i] = vmsByState[i].clone();
        }

        for (int i = 0; i < nodesByState.length; i++) {
            c.nodesByState[i] = nodesByState[i].clone();
        }          */
        return c;
    }
