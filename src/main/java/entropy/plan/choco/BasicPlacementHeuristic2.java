...
public class BasicPlacementHeuristic2 implements CorePlanHeuristic {
    ...
    public void add(ChocoCustomRP plan) {
        ...
        if (managed.isEmpty()) {
            rp.addGoal(new AssignVar(new ExcludedVirtualMachines(rp, rp.getSourceConfiguration(), vmsToExlude), new StayFirstSelector2(rp, packing, StayFirstSelector2.Option.wfMem)));
        } else {
        ...
        for (ManagedElementSet<VirtualMachine> vms : rp.getVMGroups()) {
            ...
            if (managed.isEmpty()) {
                rp.addGoal(new AssignVar(selectForInGroups, new StayFirstSelector2(rp, packing, StayFirstSelector2.Option.wfMem)));
            } else {
        ...
        if (managed.isEmpty()) {
            rp.addGoal(new AssignVar(selectForBads, new StayFirstSelector2(rp, packing, StayFirstSelector2.Option.wfMem)));
        } else {
        ...
        if (managed.isEmpty()) {
            rp.addGoal(new AssignVar(selectForGoods, new StayFirstSelector2(rp, packing, StayFirstSelector2.Option.wfMem)));
        } else {
        ...
        if (managed.isEmpty()) {
            rp.addGoal(new AssignVar(selectForRuns, new StayFirstSelector2(rp, packing, StayFirstSelector2.Option.wfMem)));
        } else {
